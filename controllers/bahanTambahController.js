...
module.exports = {
  ...
  async postDataPembelian(req, res, next) {
    ...
    try {
      let find = await models.MaterialStocks.findOne({
        where: {
          item: item,
          unit: unit,
          sellPrice: sellPrice,
      ...
      let stock;
      if (find === null) {
        stock = await models.MaterialStocks.create({
          item,
          unit,
          qty,
          sellPrice,
        ...
        stock = await models.MaterialStocks.increment("qty", {
          by: qty,
          where: {
            id: find.id,
          },
      ...
      if (order) {
        ...
        const isChanged = await models.sequelize.transaction(async (t) => {
          JSON.parse(items.itemDetail).map(async (item) => {
            await models.MaterialStocks.decrement(
              "qty",
              {
                by: item.qty,
                where: {
  ...
  async PostEditStok(req, res, next) {
    ...
    try {
      const stock = await models.MaterialStocks.update(
        {
          item: item,
          unit: unit,
          qty: qty,
